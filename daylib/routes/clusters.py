...
def _find_region_for_region_az(region_az: str, *, allowed_regions: List[str]) -> Optional[str]:
    """Find the configured region matching a region/AZ string."""
    candidates = [region for region in allowed_regions if region_az.startswith(region)]
    if not candidates:
        return None
...
def _normalise_s3_bucket_location(location: Optional[str]) -> str:
    """Normalise S3 bucket location values for region matching."""
    if not location:
        return "us-east-1"
    if location == "EU":
        return "eu-west-1"
...
class ClusterCreateRequest(BaseModel):
    """Request model for creating a ParallelCluster via daylily-ec."""

    region_az: str = Field(..., description="Availability zone, for example us-west-2a")
    cluster_name: str = Field(..., description="ParallelCluster name, max 60 chars")
    ssh_key_name: str = Field(..., description="EC2 keypair name for the headnode")
    s3_bucket_name: str = Field(..., description="S3 bucket name for daylily-ec config")
    config_path: Optional[str] = Field(None, description="Optional existing daylily-ec config YAML path")
    pass_on_warn: bool = Field(False, description="Proceed on daylily-ec warnings")
    debug: bool = Field(False, description="Enable verbose daylily-ec output")
...
def create_clusters_router(deps: ClusterDependencies) -> APIRouter:
    ...
    def _get_allowed_regions_and_profile() -> tuple[List[str], Optional[str]]:
        ...
        return (allowed_regions or []), aws_profile
    ...
    async def create_cluster(
        ...
        """Create a ParallelCluster in the background (admin-only)."""
        ...
        if not cluster_name or not _CLUSTER_NAME_PATTERN.match(cluster_name):
            raise HTTPException(
                status_code=400,
                detail=(
                    "Invalid cluster_name. Must start with a letter and contain only letters, "
                    "numbers, and '-' (max 60 chars)."
                ),
            )
        ...
        if not s3_bucket_name:
            raise HTTPException(status_code=400, detail="s3_bucket_name is required")
    ...
    async def get_cluster_create_job_logs(
        ...
        """Tail a cluster-create job log (admin-only)."""
        from daylib.file_api import _enforce_admin_only
        from daylib.ephemeral_cluster import runner as ec_runner
    ...
    async def get_cluster_create_options(
        region: str = Query(..., description="AWS region for keypair and bucket lookup"),
        current_user: Optional[Dict] = Depends(get_current_user),
    ):
        """Fetch create-form keypair and bucket options (admin-only)."""
        from daylib.file_api import _enforce_admin_only
        ...
        try:
            ...
            for bucket in response.get("Buckets", []):
                ...
                if not isinstance(name, str) or not name or "omics-analysis" not in name:
                    continue
                try:
                    location = s3.get_bucket_location(Bucket=name).get("LocationConstraint")
                    if _normalise_s3_bucket_location(location) == region:
                        buckets.append(name)
                except Exception:
                    continue
