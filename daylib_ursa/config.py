...
def _yaml_seed_from_ursa_config() -> dict[str, object]:
    ...
    seeded = {
        ...
        "auto_provision_allowed_domains": cfg.auto_provision_allowed_domains,
...
# Default tenant used for zero-tenant fallback / auto-provision flows
default_tenant_id: {DEFAULT_COGNITO_DEFAULT_TENANT_ID}

# Domains allowed for auto-provision fallback
...
class Settings(BaseSettings):
    ...
    def get_auto_provision_allowed_domains(self) -> List[str]:
        """Get list of auto-provision allowed email domains."""
        return [d.strip().lower() for d in self.auto_provision_allowed_domains if d.strip()]
    ...
    default_tenant_id: str = Field(
        default=DEFAULT_COGNITO_DEFAULT_TENANT_ID,
        description="Default tenant ID used for zero-tenant fallback / auto-provision flows",
    )
    auto_provision_allowed_domains: List[str] = Field(
        default_factory=lambda: list(DEFAULT_COGNITO_AUTO_PROVISION_ALLOWED_DOMAINS),
        description="Allowed email domains for auto-provision fallback",
    )
    ...
    @field_validator("auto_provision_allowed_domains", mode="before")
    @classmethod
    def validate_auto_provision_allowed_domains(cls, value):
        if value is None:
            return list(DEFAULT_COGNITO_AUTO_PROVISION_ALLOWED_DOMAINS)
        if isinstance(value, str):
            raw = value.split(",")
        elif isinstance(value, (list, tuple, set)):
            raw = value
        else:
            raise ValueError("auto_provision_allowed_domains must be a string or list")
        cleaned = [str(item or "").strip().lower() for item in raw if str(item or "").strip()]
        return cleaned or list(DEFAULT_COGNITO_AUTO_PROVISION_ALLOWED_DOMAINS)
