...
def _make_authenticated_client(
    mock_state_db: MagicMock, *, customer_id: str, is_admin: bool
) -> TestClient:
...
def _stub_cluster_services(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "daylily_ursa.cluster_service.get_cluster_service", lambda **kwargs: service
    )
...
_CASES = [
    ...
    _AuthzCase(
        ...
        200,
        200,
        assert_non_admin=_assert_clusters_non_admin,
        assert_admin=_assert_clusters_admin,
    ),
    ...
    _AuthzCase(
        "workset_detail_owned_user", "/portal/worksets/ws-owned-by-cust-user", 302, 200, 404
    ),
    _AuthzCase(
        "workset_detail_owned_admin", "/portal/worksets/ws-owned-by-cust-admin", 302, 404, 200
    ),
    _AuthzCase(
        "workset_detail_not_owned", "/portal/worksets/ws-owned-by-cust-other", 302, 404, 404
    ),
]
