...
from daylily_ursa.file_upload import (
    FileUploadManager,
...
def verify_file_ownership(file: Optional[FileRegistration], customer_id: str) -> bool:
    ...
    This mirrors :func:`daylily_ursa.workset_api.verify_workset_ownership` but for
    :class:`~daylily_ursa.file_registry.FileRegistration` records. Ownership is
...
def create_file_api_router(
    ...
    async def create_fileset(
        ...
        try:
            ...
            fileset = FileSet(
                fileset_id=request.name,  # use name as legacy id
                customer_id=customer_id,
                name=request.name,
            ...
            fileset_euid = file_registry.create_fileset(fileset)
            if not fileset_euid:
                raise HTTPException(
                    status_code=status.HTTP_409_CONFLICT,
                    detail=f"FileSet {request.name} already exists",
                )
    ...
    async def bulk_import_files(
        ...
        if request.fileset_name and file_ids:
            try:
                fileset = FileSet(
                    fileset_id=request.fileset_name,
                    customer_id=customer_id,
    ...
    async def get_manifest_template(
        ...
        from daylily_ursa.file_metadata import TSV_COLUMN_DEFINITIONS
