...
E_OUT_OF_MEMORY = _register_error(
    ErrorCode(
        code="WS-RES-001",
        name="OutOfMemory",
...
E_CONNECTION_REFUSED = _register_error(
    ErrorCode(
        code="WS-NET-002",
...
E_INVALID_FASTQ = _register_error(
    ErrorCode(
        code="WS-DAT-001",
        name="InvalidFASTQ",
...
E_MISSING_INPUT = _register_error(
    ErrorCode(
        code="WS-DAT-002",
        name="MissingInput",
...
E_BAD_REFERENCE = _register_error(
    ErrorCode(
        code="WS-DAT-003",
...
E_ACCESS_DENIED = _register_error(
    ErrorCode(
        code="WS-AWS-002",
        name="AccessDenied",
...
E_NODE_FAILURE = _register_error(
    ErrorCode(
        code="WS-CLU-001",
        name="NodeFailure",
...
class ErrorAnalyzer:
    ...
    def _compile_patterns(self) -> None:
        ...
        for code, error in ERROR_CODES.items():
            self._compiled_patterns[code] = [re.compile(p, re.IGNORECASE) for p in error.patterns]
    ...
    def analyze_logs(
        ...
        results.sort(
            key=lambda r: severity_order.get(
                r.error_code.severity if r.error_code else ErrorSeverity.INFO, 4
            )
        )
