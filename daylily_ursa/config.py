...
class Settings(BaseSettings):
    ...
    https_trusted_proxy_ips: str = Field(
        default="127.0.0.1,::1",
        description=(
            "Comma-separated list of proxy source IPs trusted to set X-Forwarded-Proto "
            "for HTTPS termination."
        ),
    )
    https_hsts_max_age: int = Field(
        default=31536000,
        description="HSTS max-age in seconds for secure responses",
    )
    ...
    def get_https_trusted_proxy_ips(self) -> List[str]:
        """Get list of trusted proxy source IPs for forwarded HTTPS detection."""
        return [ip.strip() for ip in self.https_trusted_proxy_ips.split(",") if ip.strip()]
