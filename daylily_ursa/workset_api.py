...
Route handlers live in daylily_ursa/routes/*.py modules.
"""
...
from daylily_ursa.config import Settings, get_settings
...
from daylily_ursa.workset_customer import CustomerManager, CustomerConfig
...
def create_app(
    ...
    trusted_proxy_ips = set(settings.get_https_trusted_proxy_ips())
    hsts_max_age = max(int(settings.https_hsts_max_age), 0)
    hsts_header_value = f"max-age={hsts_max_age}; includeSubDomains"

    def _is_https_from_trusted_proxy(request: Request) -> bool:
        client_host = request.client.host if request.client else None
        if not client_host or client_host not in trusted_proxy_ips:
            return False
        x_forwarded_proto = request.headers.get("x-forwarded-proto", "")
        if not x_forwarded_proto:
            return False
        first_proto = x_forwarded_proto.split(",")[0].strip().lower()
        return first_proto == "https"

    @app.middleware("http")
    async def enforce_https_transport(request: Request, call_next):
        is_secure_transport = request.url.scheme == "https" or _is_https_from_trusted_proxy(request)
    ...
    from daylily_ursa.exceptions import DaylilyException
    ...
    if templates_dir.exists():
        ...
        from daylily_ursa.routes.dashboard import (
        ...
        from daylily_ursa.routes.billing import (
