...
def main() -> int:
    ...
    from daylily_ursa.config import get_settings
    ...
    print(
        f"[spot-market] started at {_now_iso()} profile={aws_profile} allowed_regions={allowed_regions}"
    )
    ...
    while not _STOP:
        ...
        print(
            f"[spot-market] tick {_now_iso()} regions={regions} interval_seconds={interval_seconds}"
        )
        ...
        for region in regions[:3]:
            ...
            if region not in allowed_regions:
                ...
                continue
            try:
                p = sm_runner.compute_and_store_snapshot(
                    region=region, aws_profile=aws_profile, cfg=cfg
                )
