...
from daylily_ursa.ephemeral_cluster.runner import resolve_daylily_ec_command_prefix
...
def get_daylily_ec_resources_dir() -> Path:
    daylily_ec_cmd = resolve_daylily_ec_command_prefix()
    proc = subprocess.run(
        [*daylily_ec_cmd, "resources-dir"],
        text=True,
        capture_output=True,
        check=False,
    )
...
def start_poll_job(
    *,
    ...
    proc = subprocess.Popen(
        [sys.executable, "-m", "daylily_ursa.spot_market.job_runner", "--job-file", str(job_path)],
...
def list_jobs(*, limit: int = 20) -> List[Dict[str, Any]]:
    ...
    for p in sorted(d.glob("*.json"), key=lambda x: x.stat().st_mtime, reverse=True):
        try:
            job_doc = _read_json(p)
            job_doc = _normalize_job_state(job_doc, job_path=p)
            jobs.append(job_doc)
        except Exception:
            continue
        if len(jobs) >= limit:
...
def read_job(job_id: str) -> Dict[str, Any]:
    ...
    job_doc = _read_json(job_path)
    return _normalize_job_state(job_doc, job_path=job_path)
...
def _normalize_job_state(job: Dict[str, Any], *, job_path: Path) -> Dict[str, Any]:
    """Normalize stale running-state jobs when the runner PID is dead."""
    ...
    if isinstance(runner_pid, int):
        pid = runner_pid
    elif isinstance(runner_pid, str) and runner_pid.strip().isdigit():
        pid = int(runner_pid.strip())

    # No PID means we cannot infer liveness. Keep as-is.
    if pid is None:
        return job
    if _is_pid_running(pid):
        return job

    # Runner process disappeared without finalizing status.
    job["status"] = "failed"
    job["completed_at"] = job.get("completed_at") or _now_iso()
    if job.get("return_code") is None:
        job["return_code"] = -1
    job["error"] = job.get("error") or "Spot market poll runner process is not running"
...
def list_running_jobs(*, limit: int = 100) -> List[Dict[str, Any]]:
